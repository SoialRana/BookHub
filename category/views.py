...
from category.filters import Bookfilter
...
def BookListView(request):
    ...
    myfilter2 = Bookfilter(request.GET, queryset=booklist)

    book_list = myfilter2.qs
    context={
        "book_list":book_list,
        "filter":myfilter2
    }
    return render(request, 'category/book_list.html',context,)

@login_required
def student_BookListView(request):
    student=Student.objects.get(roll_no=request.user)
    borrower=Borrower.objects.filter(student=student)
    book_list=[]
    for b in borrower:
        book_list.append(b.book)
    return render(request, 'category/book_list.html', locals())
...
def BookDetailView(request, pk):
    ...
    try:
        ...
        pass
    return render(request, 'category/book_detail.html', locals())
...
def BookUpdate(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            obj.save()
            return redirect(index)
    return render(request, 'category/form.html', locals())
...
def student_request_issue(request, pk):
    ...
    if s.total_books_due < 10:
        message = "book has been isuued, we can collect book from library"
        a = Borrower()
        a.student = s
        a.book = obj
...
def StudentList(request):
    ...
    return render(request, 'category/student_list.html', locals())

@login_required
def StudentDetail(request, pk):
    student = get_object_or_404(Student, id=pk)
    books=Borrower.objects.filter(student=student)
    return render(request, 'category/student_detail.html', locals())
